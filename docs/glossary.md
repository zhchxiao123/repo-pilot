...
| **Eval case** | One pinned repo + its expected canonical verdict (`verified:<shape>`, `not_runnable:docs`, `deferred`, …) in an eval manifest. The pinned 50-case manifest (`eval/manifest.50.json`) is the source of truth for coverage milestones. |
| **Verdict coverage** | Fraction of eval cases whose actual verdict matches the expected one (hierarchically: `verified` subsumes `verified:cli`). Misses are grouped into **failure clusters** by `expected->actual`, each pointing at the case's persisted artifacts (`docs/eval-harness.md`). |
